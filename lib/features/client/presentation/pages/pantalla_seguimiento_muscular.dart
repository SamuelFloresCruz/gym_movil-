...
class ClientMuscleTrackingPage extends StatefulWidget {
  const ClientMuscleTrackingPage({super.key});
...
class _ClientMuscleTrackingPageState extends State<ClientMuscleTrackingPage> {
  ...
  late final DateTime _visibleMonth = DateTime(
    DateTime.now().year,
    DateTime.now().month,
  );
  ...
  Future<void> _openEntryForm({TrackingEntry? entry, int? entryIndex}) async {
    final savedEntry = await showModalBottomSheet<TrackingEntry>(
      ...
      builder: (context) {
        return _TrackingEntrySheet(day: _selectedDay, initialEntry: entry);
      },
    );
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Row(
            children: [
              Icon(Icons.calendar_month_outlined, color: ClientColors.red),
              SizedBox(width: 10),
              Expanded(
                child: Text(
...
class _TrackingEntrySheet extends StatefulWidget {
  const _TrackingEntrySheet({required this.day, this.initialEntry});
...
class _TrackingEntrySheetState extends State<_TrackingEntrySheet> {
  ...
  TrainingMuscle get _selectedMuscle {
    return ClientTrainingCatalog.muscles[_selectedMuscleIndex];
  }
  ...
  void initState() {
    ...
    if (entry != null) {
      final muscleIndex = ClientTrainingCatalog.muscles.indexWhere(
  ...
  void _save() {
    ...
    if (exercise.isEmpty) {
      ScaffoldMessenger.of(context)
        ...
        ..showSnackBar(
          const SnackBar(
            content: Text('Agrega o selecciona un ejercicio.'),
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ListView.separated(
                ...
                itemCount: ClientTrainingCatalog.muscles.length,
                separatorBuilder: (_, _) => const SizedBox(width: 10),
                itemBuilder: (context, index) {
                  final muscle = ClientTrainingCatalog.muscles[index];
